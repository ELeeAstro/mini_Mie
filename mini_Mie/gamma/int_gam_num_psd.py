...
def gam_num_psd(r, N, lam, m, alpha, beta):
  ...
  # Find number density per size (size distribution) [cm-3 cm-1], assuming gamma distribution - vectorised
  f_r = np.empty(lr)
  f_r[:] = N/(gamma(alpha) * beta_cm**alpha) * r_cm[:]**(alpha-1.0) * np.exp(-r_cm[:]/beta_cm)
